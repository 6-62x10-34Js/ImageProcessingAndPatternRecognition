...
def diff_tens(
    ...
    # dx, dy = (nabla @ u_tilde.ravel()).reshape(2, *u.shape)
    ...
    # imageio.imsave(f'./dx.png', (dx * 255).astype(np.uint8))
    # imageio.imsave(f'./dy.png', (dy * 255).astype(np.uint8))

    # u_x_tilde = gaussian_filter(dx, sigma_u)
    # u_y_tilde = gaussian_filter(dy, sigma_u)
...
if __name__ == "__main__":
    ...
    # input = imageio.imread(inputs[mode]).mean(-1) / 255.
    output = nonlinear_anisotropic_diffusion(
        input, **params[mode], mode=mode
    )
    imageio.imsave(f'./{mode}_out_Day3.png', (output * 255).astype(np.uint8))
